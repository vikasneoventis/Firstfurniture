...
class Magento19 extends \LitExtension\CartMigration\Model\Cart
{
    ...
    public function convertManufacturer($manufacturer, $manufacturersExt) {
        ...
        $manufacturer_data['value']['option'] = array(
            0 => isset($manu_store['0']) ? $manu_store['0'] : $manu_store['1']
        );
    ...
    protected function _getProductsMainQuery() {
		...
		//$query = "SELECT * FROM _DBPRF_catalog_product_entity WHERE entity_id = 208";
        return $query;
    }
    ...
    public function getProductsExt($products) {
        ...
        $ext_query = array(
            ...
            'catalog_product_entity_media_gallery' => "SELECT * FROM _DBPRF_catalog_product_entity_media_gallery WHERE entity_id IN {$pro_id_query}",
		...
		$imgIds = $this->duplicateFieldValueFromList($productsExt['object']['catalog_product_entity_media_gallery'], 'value_id');
        $img_ids_query = $this->arrayToInCondition($imgIds);
        $ext_rel_query = array(
			'catalog_product_entity_media_gallery_value' => "SELECT * FROM _DBPRF_catalog_product_entity_media_gallery_value WHERE value_id IN {$img_ids_query} AND store_id = 0",
    ...
    public function convertProduct($product, $productsExt) {
		...
		if(!in_array($product['entity_id'], array(54421))){
		//var_dump($product['entity_id']);exit;
				...
				foreach ($gallery as $img) {
					if ($img['value']) {
						...
						if (!$this->_notice['extend']['migrate_image']) {
							...
							$pro_data['image_gallery'][] = array('path' => $gallery_path, 'label' => $this->getRowValueFromListByField($productsExt['object']['catalog_product_entity_media_gallery_value'], 'value_id', $img['value_id'], 'value'), 'exclude' => $this->getRowValueFromListByField($productsExt['object']['catalog_product_entity_media_gallery_value'], 'value_id', $img['value_id'], 'disabled'), 'media' => $media);
						}
					}
				}
			}
