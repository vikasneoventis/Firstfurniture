...
class LECM_Connector_Adapter_Magento extends LECM_Connector_Adapter {
   function __construct() {
        ...
        if(file_exists(LECM_STORE_BASE_DIR . 'app' . DIRECTORY_SEPARATOR . 'etc' . DIRECTORY_SEPARATOR . 'env.php')){
            $config = include_once(LECM_STORE_BASE_DIR . 'app' . DIRECTORY_SEPARATOR . 'etc' . DIRECTORY_SEPARATOR . 'env.php');
            $this->setHostPort($config['db']['connection']['default']['host']);
            $this->Username = $config['db']['connection']['default']['username'];
            $this->Password = $config['db']['connection']['default']['password'];
            $this->Dbname = $config['db']['connection']['default']['dbname'];
            ...
            $composer = file_get_contents(LECM_STORE_BASE_DIR . 'composer.json');
